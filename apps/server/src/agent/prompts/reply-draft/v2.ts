...
/**
 * Deterministic relationship → register. The application sets the frame; the model only
 * fills in the words (the Phase 7 principle, applied to style).
 *
 * UNKNOWN and OTHER are deliberately absent: UNKNOWN never reaches drafting at all (the
 * contact policy blocks it before any AI call), and OTHER carries no reliable signal — so
 * we assert nothing about the relationship rather than guess, and the system prompt's
 * "no register given" default applies.
 */
...
// Mirrors the contact API's notes cap. Applied again here because build() is pure and
// reads straight from the DB, which can hold longer legacy or hand-edited values.
...
export const replyDraftV2: PromptTemplate = {
  id: "reply-draft",
  version: "v2",
  build
};
